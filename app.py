...
def show_admin_dashboard():
    ...
    tab_inventory, tab_stock, tab_accounts, tab_override = st.tabs(
        [
            "📊 System Inventory",
            "➕ Stock Management",
            "👤 Account Management",
    ...
    with tab_stock:
        ...
        if add_submitted:
            if not new_title.strip():
                ...
                st.success(f"Added '{new_title.strip()}' to inventory.")
                st.rerun()
        ...
        if existing_titles:
            ...
            if update_submitted:
                ...
                storage_manager.save_books()
                st.success(f"Added {delta} copy/copies of '{selected_title}'.")
                st.rerun()
        else:
    ...
    with tab_accounts:
        ...
        if create_submitted:
            try:
                new_user = library.add_new_user(first_name.strip(), last_name.strip(), is_admin=grant_admin)
                storage_manager.save_users()
                st.success(
